...
import { SwiftXTokenAbi, SwiftXTokenAddress } from "../_lib/abi";
import { provider } from "../_lib/cb_provider";
...
export default function Home() {
  ...
  const capabilities = useMemo(() => {
    ...
    if (
      ...
      return {
        paymasterService: {
          url: `https://api.developer.coinbase.com/rpc/v1/base-sepolia/MdtgmmnfpOHqviXsQ3SsOuEIEPvQTI5l`,
        },
      };
    }
    return {};
        ...
        <Table
          ...
          onBurnToken={async (data: { amount: number; txId: string }) => {
            ...
            writeContracts({
              contracts: [
                {
                  address: SwiftXTokenAddress,
                  abi: SwiftXTokenAbi,
                  functionName: "confirmTransfer",
                  args: [data.amount, data.txId, account.address],
                },
              ],
              capabilities,
            });
